...
class YOLOLoss(nn.Module):
    def __init__(
        ...
        # self.box_ratio      = 1.0
        # self.obj_ratio      = 1.0
        self.cls_ratio      = 0.5 * (num_classes / 20)
    ...
    def forward(self, l, input, targets=None, y_true=None, batch=None):
        self.input_shape = (batch[0].shape[3], batch[0].shape[2])
        #----------------------------------------------------#
        #   l               代表使用的是第几个有效特征层
        ...
        # print(f'[forward]  {scaled_anchors=}')
        #-----------------------------------------------#
        ...
        # print(f'[forward]  {prediction.shape=}')
        ...
        # print(f'[forward]  {y_true.device=}  {x.device=}  {prediction.device=}')
        # if self.cuda:
        ...
        # print(f'[==>]  {tobj.min()=}  {tobj.max()=}  {conf.min()=}  {conf.max()=}')
    ...
    def get_pred_boxes(self, l, x, y, h, w, targets, scaled_anchors, in_h, in_w):
        ...
        # print(f'[get_pred_boxes]  {grid_x.shape=}  {grid_y.shape=}')
        #-----------------------------------------------------#
        #   生成先验框的宽高
        #   grid_x： <b, 3, h, w>
        #   grid_y： <b, 3, h, w>
        #   scaled_anchors_l  <3, 2>：
        #       [[ 3.625  ,  2.8125 ],
        #       [ 4.875  ,  6.1875 ],
        #       [11.65625, 10.1875 ]]
        #-----------------------------------------------------#
        ...
        # print(f'[get_pred_boxes]  {scaled_anchors_l=}')
        # print(f'[get_pred_boxes]  {anchor_w=}  {anchor_w.shape=}  {anchor_h=}  {anchor_h.shape=}')
        
        # anchor_w: [B, 3, h, w]
        # anchor_h: [B, 3, h, w]
        anchor_w = anchor_w.repeat(bs, 1).repeat(1, 1, in_h * in_w).view(w.shape)
        anchor_h = anchor_h.repeat(bs, 1).repeat(1, 1, in_h * in_w).view(h.shape)
        # print(f'[get_pred_boxes]  {anchor_w.shape=}  {anchor_h.shape=}')
        #-------------------------------------------------------#
        #   计算调整后的先验框中心与宽高
        #   pred_boxes: [B, 3, 20, 20, 4]
        #-------------------------------------------------------#
        ...
        # print(f'[get_pred_boxes]  {pred_boxes.shape=}')
        return pred_boxes
