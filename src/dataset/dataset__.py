...
class YoloDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # img_ = rectangles(image.copy(), BBoxes(box, mode='xyxy'))
        # save_image(img_, '/data/ylw/code/yolo/yolov5-pytorch/test/test_rets/0.jpg')

        image       = np.transpose(preprocess_input(np.array(image, dtype=np.float32)), (2, 0, 1))
        box         = np.array(box, dtype=np.float32)
        box_ = deepcopy(box)
        # img_ = (image.copy() * 255).astype(np.int8).transpose((1,2,0))
        # img_ = rectangles(cv2.UMat(img_), BBoxes(box, mode='xywh'), copy=False)
        # save_image(img_, '/data/ylw/code/yolo/yolov5-pytorch/test/test_rets/test.jpg')
        # sllog << f'[__getitem__]  {img_.shape=}  {box.shape=}  {box.max()=}  {box.min()=}  {box=}'
        ...
        # save_image(img_, '/data/ylw/code/yolo/yolov5-pytorch/test/test_rets/test_1.jpg')
        # img_ = cv2.addWeighted(img_.astype(np.uint8), 0.5, tmp, 0.5, 1)
        # img_ = rectangles(cv2.UMat(img_), BBoxes(box_, mode='xyxy'), copy=False)
        # save_image(img_, '/data/ylw/code/yolo/yolov5-pytorch/test/test_rets/test_2.jpg')
    ...
    def get_random_data_with_Mosaic(self, annotation_line, input_shape, jitter=0.3, hue=.1, sat=0.7, val=0.4):
        ...
        # save_image(image_datas[0], f'/data/ylw/code/yolo/yolov5-pytorch/test/test_rets/0.jpg')
        # save_image(image_datas[1], f'/data/ylw/code/yolo/yolov5-pytorch/test/test_rets/1.jpg')
        # save_image(image_datas[2], f'/data/ylw/code/yolo/yolov5-pytorch/test/test_rets/2.jpg')
        # save_image(image_datas[3], f'/data/ylw/code/yolo/yolov5-pytorch/test/test_rets/3.jpg')
        # save_image(new_image, f'/data/ylw/code/yolo/yolov5-pytorch/test/test_rets/4.jpg')
        ...
        # img_ = rectangles(new_image.copy(), BBoxes(new_boxes, mode='xyxy'))
        # save_image(img_, '/data/ylw/code/yolo/yolov5-pytorch/test/test_rets/0.jpg')
    ...
    def get_target(self, targets):
        ...
        y_true = [np.zeros((len(self.anchors_mask[l]), grid_shapes[l][0], grid_shapes[l][1], self.bbox_attrs)) for l in range(num_layers)]
        #-----------------------------------------------------------#
    ...
    def yolo_dataset_collate(self, batch):
        ...
        self.input_shape = (input_shape, input_shape)
        # print(f'[yolo_dataset_collate]  set {self.input_shape=}')
