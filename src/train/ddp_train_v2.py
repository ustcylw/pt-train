...
def run_worker(local_rank, world_size):
    ...
    for epoch in range(CONFIGS.TRAIN_START_EPOCHS, CONFIGS.TRAIN_EPOCHS):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            if batch_idx % 10 == 0:
                sllog << f'[TRAIN][{local_rank}][{batch_idx}]  loss: {loss_all.item():.4f}  loss-1: {losses[0]:.4f}  loss-2: {losses[1]:.4f}  loss-3: {losses[2]:.4f}'
        ...
        with torch.no_grad():
            for batch_idx, batch in enumerate(val_loader):
                ...
                if batch_idx % 10 == 0:
                    sllog << f'[EVAL][{local_rank}]  loss: {loss_all.item():.4f}  loss-1: {losses[0]:.4f}  loss-2: {losses[1]:.4f}  loss-3: {losses[2]:.4f}'
